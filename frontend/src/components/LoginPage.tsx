...
/** Same-site relative path to return to after login (e.g. a tunnel authorize URL). */
...
export default function LoginPage({ onLegacyLogin }: Props) {
  ...
  if (remember) ghParams.set('remember', 'true')
  const returnTo = getReturnTo()
  if (returnTo) ghParams.set('return_to', returnTo)
  const ghQuery = ghParams.toString()
  ...
  return (
    <div className="h-full bg-[var(--bg-primary)] flex items-center justify-center">
      <div className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg p-8 w-80 space-y-5">
        ...
        {authMode?.oauth && (
          <a
            href={`/auth/github${ghQuery ? `?${ghQuery}` : ''}`}
